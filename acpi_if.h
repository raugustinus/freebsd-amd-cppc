...
typedef ACPI_STATUS (
    *acpi_scan_cb_t)(ACPI_HANDLE h, device_t *dev, int level, void *arg);
...
static __inline int
ACPI_ID_PROBE(device_t bus, device_t dev, char **ids, char **match)
{
	kobjop_t _m;
	int rc;
...
typedef ACPI_STATUS acpi_evaluate_object_t(device_t bus, device_t dev,
    ACPI_STRING pathname, ACPI_OBJECT_LIST *parameters, ACPI_BUFFER *ret);

static __inline ACPI_STATUS
ACPI_EVALUATE_OBJECT(device_t bus, device_t dev, ACPI_STRING pathname,
    ACPI_OBJECT_LIST *parameters, ACPI_BUFFER *ret)
{
	kobjop_t _m;
	ACPI_STATUS rc;
	KOBJOPLOOKUP(((kobj_t)bus)->ops, acpi_evaluate_object);
	rc = ((acpi_evaluate_object_t *)_m)(bus, dev, pathname, parameters,
	    ret);
	return (rc);
}
...
static __inline ACPI_STATUS
ACPI_GET_PROPERTY(device_t bus, device_t dev, ACPI_STRING propname,
...
static __inline int
ACPI_PWR_FOR_SLEEP(device_t bus, device_t dev, int *dstate)
{
	kobjop_t _m;
	int rc;
...
static __inline ACPI_STATUS
ACPI_SCAN_CHILDREN(device_t bus, device_t dev, int max_depth,
...
static __inline int
ACPI_GET_FEATURES(driver_t *driver, u_int *features)
{
	kobjop_t _m;
	int rc;
...
static __inline int
ACPI_EC_READ(device_t dev, u_int addr, UINT64 *val, int width)
{
	kobjop_t _m;
	int rc;
...
static __inline int
ACPI_EC_WRITE(device_t dev, u_int addr, UINT64 val, int width)
{
	kobjop_t _m;
	int rc;
...
static __inline int
ACPI_BATT_GET_INFO(device_t dev, void *bix, size_t len)
{
	kobjop_t _m;
	int rc;
...
static __inline int
ACPI_BATT_GET_STATUS(device_t dev, struct acpi_bst *bst)
{
	kobjop_t _m;
	int rc;
